...
template<typename T>
py::array_t<T> vector_to_np(const std::vector<T> & v){
...
PYBIND11_MODULE(invind, m) {
  ...
  py::class_<Indexer>(m, "Indexer")
      ...
      .def("query_execute", [](const Indexer &indexer, const nl::json & query){
          auto result_vector = indexer.query_execute(query);
          return vector_to_np(result_vector);
      })
      .def("query_execute_batch", [](const Indexer &indexer, const std::vector<nl::json> & queries, size_t n_thread){
          ...
          std::vector<std::future<std::vector<std::pair<uint64_t, std::vector<uint64_t>>>>> worker_results;
